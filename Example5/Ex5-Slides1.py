...
y = LpVariable.dicts("y",(stores,customers),0,1,LpInteger)

#Specify the objective statement
prob += lpSum([d[j]*c[i][j]*y[i][j] for i in stores for j in customers]), "Sum of Transporting Costs"

# Add the constraint that the maximum store count to meet demand must be equal to 1
for j in customers:
    prob += lpSum([y[i][j] for i in stores]) == 1, "Customer %s served from a single store"%j

# Add the constraint that the total customers demand from each store does not
# exceed the total inventory
for i in stores:
    prob += lpSum([d[j]*y[i][j] for j in customers]) <= k[i], "Sum of Products out of Store %s"%i
...
for i in stores:
    for j in customers:
        if y[i][j].varValue > 0:
            print(f'y {i},{j}:',y[i][j].varValue,y[i][j].varValue*d[j])