...
O = np.array([
    [10.5,12.5,12.5,24.5,28,28],
    [14.5,15.5,15.75,19.75,29.75,29.5],
    [26,24,24,26,28,23],
    [24.5,24.5,24.5,25,26.5,27.5],
    [12,12,14.5,14.5,18.5,18.5]
    ])
...
kx = pulp.LpVariable.dicts("kx",(N,T),0,None,pulp.LpInteger)

prob += pulp.lpSum([C[n,t]*I[n][t+1]+L[n,t]*x[n][t]+O[n,t]*kx[n][t] for n in N for t in T]), "Sum of labor and storage cost"
...
for n in N:
    for t in T:
        prob += pulp.lpSum(h[n]*x[n][t]-kx[n][t]) <= k[n], f'subject to labor [{n},{t}]'
